...
def deployment_decision(data, output_dir="reports/research", min_train_days=365, test_days=120, candidates=None, n_sides_grid=(3,), vol_target_annual=0.20, rebalance_every=5, fee_rate=0.0004, slippage_rate=0.0005, require_funding=False, spread_check_passed=False, delist_mode="error", ensemble_top_k=0, base_config=None):
    ...
    base = base_config or CheckerConfig(fee_rate=fee_rate, slippage_rate=slippage_rate,
                                        rebalance_every=rebalance_every,
                                        require_funding=require_funding,
                                        delist_mode=delist_mode,
                                        vol_target_annual=vol_target_annual)
    wf = walk_forward(data, base_config=base, min_train_days=min_train_days,
                      test_days=test_days, candidates=wf_candidates,
                      n_sides_grid=n_sides_grid, vol_target_annual=base.vol_target_annual,
                      output_dir=output_dir, ensemble_top_k=ensemble_top_k)
    best_signal = pd.Series([f["signal"] for f in wf["folds"]]).mode().iloc[0]
    best_n = int(pd.Series([f["n_sides"] for f in wf["folds"]]).mode().iloc[0])
    config = replace(base, n_long=best_n, n_short=best_n, signal_column=best_signal)
    ...
    decision = {
        ...
        "best_n_sides": best_n,
        "config": {"n_long": best_n, "n_short": best_n,
