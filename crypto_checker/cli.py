...
parser.add_argument("--cost-preset", default="liquid", choices=["liquid", "midcap"], help="liquid=flat 4+5bps; midcap=tiered by trailing volume percentile")
args = parser.parse_args()

liquidity_tiers = ()
if args.cost_preset == "midcap":
    liquidity_tiers = ((0.75, 0.0004, 0.0005), (0.5, 0.0008, 0.001), (0.25, 0.0012, 0.002), (0.0, 0.0015, 0.004))
...
config = CheckerConfig(n_long=args.n_long, n_short=args.n_short, fee_rate=args.fee_rate, slippage_rate=args.slippage_rate, signal_lookback=args.signal_lookback, min_signal_gap=args.min_signal_gap, signal_column=args.signal, enforce_risk_limits=args.enforce_risk_limits, liquidity_column=args.liquidity_column, liquidity_tiers=liquidity_tiers, require_funding=True)
